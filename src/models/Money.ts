...
export class Money {
  ...
  private internalCurreny: Currency<number>;
  private internal: Dinero<number>;

  constructor(n: number, currency: string) {
    this.currency = currency;
    this.internalCurreny = this.getCurrency(currency);

    const scale = this.getInitScale();
    const factor = this.internalCurreny.base ** scale;
    const amount = Math.round(n * factor);
    this.internal = dinero({ amount: amount, currency: this.internalCurreny, scale });
  }

  private getInitScale = () => {
    const zeroMoney = dinero({ amount: 0, currency: this.internalCurreny });
  ...
  public getValue = (): number => {
    return toUnit(this.internal, { digits: this.getPrecision(), round: halfEven });
  };
  ...
  public percentage = (n: number): Money => {
    this.internal = multiply(this.internal, { amount: n, scale: this.getPrecision() });
    return this;
  };
}
